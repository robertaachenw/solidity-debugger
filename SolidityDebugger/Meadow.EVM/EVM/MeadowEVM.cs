...
namespace Meadow.EVM
{
    public class MeadowEVM
    {
        ...
        public static EVMExecutionResult CreateContract(
            State state,
            EVMMessage message)
        {
            return CreateContractInternal(state, message, 1, BigInteger.Zero);
        }

        public static EVMExecutionResult CreateContract2(
            State state,
            EVMMessage message,
            BigInteger salt)
        {
            return CreateContractInternal(state, message, 2, salt);
        }
        
        
        private static EVMExecutionResult CreateContractInternal(
            State state, 
            EVMMessage message, 
            int opcodeVersion,
            BigInteger saltV2)
        {
            ...
            if (Shared.Globals.NextCreateAddress != BigInteger.Zero)
            {
                message.To = Shared.Globals.NextCreateAddress;
                Shared.Globals.NextCreateAddress = BigInteger.Zero;
            }
            else
            {
                switch (opcodeVersion)
                {
                    case 1:
                        var newNonce = state.GetNonce(message.Sender) - 1;
                        message.To = Address.MakeContractAddress(message.Sender, newNonce);
                        break;
                    
                    case 2:
                        message.To = Address.MakeContractAddress2(message.Sender, saltV2, message.Data);
                        break;
            ...
            var balance = state.GetBalance(message.To);
            if (balance > 0)
            {
        ...
        private void Step()
        {
            ...
            var opcode = (InstructionOpcode)Code.Span[(int)ExecutionState.PC];

            // Obtain our base cost for this opcode. If we fail to, the instruction isn't implemented yet.
            var instructionBaseGasCost = GasDefinitions.GetInstructionBaseGasCost(State.Configuration.Version, opcode);
            ...
            ExecutionTracePoint tracePoint = null;
            
            if (State.Configuration.DebugConfiguration.IsTracing)
            {
                tracePoint = State.Configuration.DebugConfiguration.ExecutionTrace?.RecordExecution(
                    this, 
                    instruction, 
                    GasState.Gas, 
                    (BigInteger)instructionBaseGasCost);
            }
            ...
            var logsCountBefore = State.TransactionLogs.Count;
            
            instruction.Execute();
            
            var logsCountAfter = State.TransactionLogs.Count;

            if (logsCountAfter > logsCountBefore && tracePoint != null)
            {
                tracePoint.Emit = State.TransactionLogs[logsCountAfter - 1];
            }
        }
    }
}
