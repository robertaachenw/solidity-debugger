...
namespace Meadow.SolCodeGen
{
    ...
    {        
        ...
        {
            _logger = logger;
            _assemblyVersion = typeof(OldProgram).Assembly.GetName().Version.ToString();
        ...
        void GenerateSources()
        {
            ...
            {
                solFiles = GetSolContractFiles(_solSourceDirectory).ToArray();
            }
            ...
            var solcOutput = _solcLibPortable.Compile(
                solFiles, 
                outputFlags, 
                solcOptimizerSettings, 
                soliditySourceFileContent: soliditySourceContent
                );
        ...
        static List<string> GetSolContractFiles(
            string contractsDir,
            bool allowEmpty = false)
        {
            ...
            if (solFiles.Count == 0 && !allowEmpty)
            {
