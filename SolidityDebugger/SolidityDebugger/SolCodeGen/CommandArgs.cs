    ...
    public class CommandArgs
    {
        ...
        public static bool TryParse(string[] args, LoggerDelegate logger, out CommandArgs argResult, out Exception parseException)
        {
            try
            {
                ...
                var solcDllPathOption = app
                    .Option("-a|--solcJsPath", "Solidity compiler (soljson.js)", CommandOptionType.SingleValue)
                    .OnValidate(ctc =>
                    {
                        var opt = (ctc.ObjectInstance as CommandOption)?.Value();
                        if (!string.IsNullOrWhiteSpace(opt))
                        {
                            if (!File.Exists(opt))
                            {
                                return new ValidationResult($"File not found: {opt}");
                            }

                            result.SolcJsPath = opt;
                            logger($"Solidity compiler dll: {opt}");
                        }
