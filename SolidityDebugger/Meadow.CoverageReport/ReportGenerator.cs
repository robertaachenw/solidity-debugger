...
namespace Meadow.CoverageReport
{
    public static class ReportGenerator
    {
        ...
        private static Dictionary<int, SourceFileMap> CreateSourceFileMapsParallel(
            SolcSourceInfo[] sourcesList,
            AnalysisResults analysis)
        {
            var threadPool = new SmartThreadPool();
            
            var sourceIndexes = new ConcurrentDictionary<int, SourceFileMap>();
            var workItems = new List<IWorkItemResult>();
            for (var j = 0; j < sourcesList.Length; ++j)
            {
                var workItem = threadPool.QueueWorkItem((int i) => 
                    {
                        var solSource = sourcesList[i];
            ...
            SmartThreadPool.WaitAll(workItems.ToArray());

            return sourceIndexes.ToDictionary(
                entry => entry.Key, 
                entry => entry.Value);
        }
        ...
        public static Dictionary<int, SourceFileMap> CreateSourceFileMaps(
            ...
            AnalysisResults analysis)
        {
            return CreateSourceFileMapsParallel(sourcesList, analysis);
        }
