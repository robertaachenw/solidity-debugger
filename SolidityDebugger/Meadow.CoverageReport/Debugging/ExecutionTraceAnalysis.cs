...
using Data = Meadow.Core.EthTypes.Data;
...
namespace Meadow.CoverageReport.Debugging
{
    ...
    public class ExecutionTraceAnalysis
    {
        ...
        /// <summary>
        /// Emitted event logs by trace point index.
        ///
        /// Events[tracePointIndex] = (
        ///     string eventName,
        ///     Dictionary[eventArgumentName] -> (string eventArgumentType, string eventArgumentValue) 
        /// )
        /// </summary>
        public Dictionary<int, (string eventName, Dictionary<string, (string type, string value)>)> Events;
        ...
        class PatchExecutionTraceAnalysis : IPatchExecutionTraceAnalysis
        {
            public List<SolcBytecodeInfo> SolcBytecodeInfos { get; set; }
            public List<SolcSourceInfo> SolcSourceInfos { get; set; }
        ...
        public ExecutionTraceAnalysis(ExecutionTrace executionTrace)
        {
            ...
            _contractMapCache = new Dictionary<
                (Address ContractAddress, string CodeHash, bool Deployed), 
                (SourceMapEntry[] SourceMap, Dictionary<int, int> InstructionOffsetToNumber)
            >();
            ...
            // Allow plugin mods
            var patch = new PatchExecutionTraceAnalysis
            {
                SolcSourceInfos = _solcData.SolcSourceInfo.ToList(),
                SolcBytecodeInfos = _solcData.SolcBytecodeInfo.ToList(),
                Reload = false
            };

            try
            {
                PluginLoader.Default.OnExecutionTraceAnalysis(null, patch);

                if (patch.Reload)
                {
                    // update solc data
                    _solcData = (patch.SolcSourceInfos.ToArray(), patch.SolcBytecodeInfos.ToArray());
                    GeneratedSolcData.Default.SetSolcData(_solcData.SolcSourceInfo, _solcData.SolcBytecodeInfo);
                    
                    // parse source code again
                    AstParser.IsParsed = false;
                    _analysisResults = SourceAnalysis.Run(_solcData.SolcSourceInfo, _solcData.SolcBytecodeInfo);
                    _sourceFileMaps = ReportGenerator.CreateSourceFileMaps(_solcData.SolcSourceInfo, _analysisResults);
                    AstParser.Parse();
                }
            ...
            // PluginLoader.Default.Svc.Log($"[ExecutionTraceAnalysis] ExecutionTrace.Tracepoints.Length={ExecutionTrace.Tracepoints.Length}");
            // PluginLoader.Default.Svc.Log("[ExecutionTraceAnalysis] Enter ParseScopes()");
            PluginLoader.Default.Svc.Log("Parsing scopes");
            ParseScopes(0, 0, 0, null, ref lastEntry);
            // PluginLoader.Default.Svc.Log("[ExecutionTraceAnalysis] Leave ParseScopes()");

            ParseEmittedEvents();
            
            PluginLoader.Default.Svc.StopBenchmark();
            // PluginLoader.Default.Svc.PrintBenchmarks();
        }
        //#endregion

        //#region Functions
        
        private void ParseEmittedEvents()
        {
            // extract event definitions from AST
            var hashToEventName = new Dictionary<string, string>();
            var hashToEventArgTypes = new Dictionary<string, string[]>();
            var hashToEventArgNames = new Dictionary<string, string[]>();
            
            foreach (var node in AstParser.AllAstNodes)
            {
                try
                {
                    var nodeType = node.Node["nodeType"].ToString();
                    if (!nodeType.Equals("EventDefinition", StringComparison.OrdinalIgnoreCase))
                        continue;

                    var eventName = node.Node["name"].ToString();

                    
                    var eventParamNames = new List<string>();
                    var eventParamTypes = new List<string>();
                    var i = 0;
                    while (true)
                    {
                        try
                        {
                            eventParamNames.Add(node.Node["parameters"]["parameters"][i]["name"].ToString());
                            eventParamTypes.Add(node.Node["parameters"]["parameters"][i]["typeDescriptions"]["typeString"].ToString());
                        }
                    ...
                    var eventSig = $"{eventName}({string.Join(',', eventParamTypes)})";
                    var eventSigHash = $"0x{KeccakHash.FromString(eventSig).ToHexString()}";

                    hashToEventName[eventSigHash] = eventName;
                    hashToEventArgNames[eventSigHash] = eventParamNames.ToArray();
                    hashToEventArgTypes[eventSigHash] = eventParamTypes.ToArray();
                }
                catch (Exception ex)
                {
                    // ignored
            ...
            // parse trace points
            var events = new Dictionary<int, (string eventName, Dictionary<string, (string type, string value)>)>();
            
            for (var tracePointIndex = 0; tracePointIndex < ExecutionTrace.Tracepoints.Length; ++tracePointIndex)
            {
                if (ExecutionTrace.Tracepoints[tracePointIndex].Emit == null) continue;
                var emit = ExecutionTrace.Tracepoints[tracePointIndex].Emit;
                if (1 > emit.Topics.Length) continue;
                var hash = emit.Topics[0];
                if (!hashToEventName.ContainsKey(hash)) continue;

                var eventName = hashToEventName[hash];
                var argNames = hashToEventArgNames[hash];
                var argTypes = hashToEventArgTypes[hash];

                try
                {
                    var eventArgs = ParseEmittedEventInternal(eventName, argNames, argTypes, emit);
                    events[tracePointIndex] = (eventName, eventArgs);
                }
                catch (Exception ex)
                {
                    // Debug.WriteLine($"ParseEmittedEvents exception at tracePointIndex={tracePointIndex} {ex}");
                }
            }
        ...
        private static Dictionary<string, (string type, string value)> ParseEmittedEventInternal(
            string eventName,
            IReadOnlyList<string> names,
            IReadOnlyList<string> types,
            ExecutionTracePointEmit trace)
        {
            var results = new Dictionary<string, (string, string)>();
            
            // convert trace data to UInt256s
            const int UINT256_SIZE = 32;
            
            if (0 != trace.Data.Length % UINT256_SIZE)
            {
                throw new Exception(@"Event emit data incorrect size");
            }

            var data = new List<UInt256>();
            for (var i = 0; i < trace.Data.Length; i += UINT256_SIZE)
            {
                var x = new Span<byte>(trace.Data.Slice(i, i + UINT256_SIZE));
                x.Reverse();
                data.Add(new UInt256(x));
            }
            
            // parse trace data
            for (var i = 0; i < Math.Min(types.Count, data.Count); ++i)
            {
                var argName = names[i];
                var argType = types[i];

                if (string.IsNullOrEmpty(argName) || string.IsNullOrEmpty(argType))
                {
                    continue;
                }

                if (argType.Equals("string", StringComparison.InvariantCulture) || 
                    argType.Equals("bytes", StringComparison.InvariantCulture))
                {
                    var offset = int.Parse(
                        data[i].ToBigInteger().ToString(CultureInfo.InvariantCulture), 
                        NumberStyles.Integer, CultureInfo.InvariantCulture);

                    var size = int.Parse(
                        data[offset / UINT256_SIZE].ToBigInteger().ToString(CultureInfo.InvariantCulture),
                        NumberStyles.Any, CultureInfo.InvariantCulture);

                    var value = trace.Data.Slice(offset + UINT256_SIZE, offset + UINT256_SIZE + size);

                    if (argType.Equals("string", StringComparison.InvariantCulture))
                    {
                        results[argName] = (argType, $"\"{Encoding.UTF8.GetString(value)}\"");
                    }
                    else
                    {
                        results[argName] = (argType, BitConverter.ToString(value).Replace("-", string.Empty));
                    }
                }
                else
                {
                    if (data[i] < 0x100000000)
                    {
                        results[argName] = (argType, data[i].ToBigInteger().ToString(CultureInfo.InvariantCulture));
                    }
                    else
                    {
                        results[argName] = (argType, data[i].ToHexString());
                    }
                }
            }
        ...
        private Dictionary<(ulong, bool), (bool, SolcBytecodeInfo)> _getInstructionAndSourceMapCache =
            new Dictionary<(ulong, bool), (bool, SolcBytecodeInfo)>();
        ...
        public (int InstructionIndex, SourceMapEntry SourceMapEntry) GetInstructionAndSourceMap(int traceIndex)
        {
            ...
            else
            {
                ...
                var xxhash = XXH64.DigestOf(tracepoint.Code);
                if (_getInstructionAndSourceMapCache.ContainsKey((xxhash, tracepoint.ContractDeployed)))
                {
                    (ret, info) = _getInstructionAndSourceMapCache[(xxhash, tracepoint.ContractDeployed)];
                }
                else
                {
                    codeHex = tracepoint.Code.ToHexString(hexPrefix: false);
                    ret = GeneratedSolcData.Default.GetSolcBytecodeInfoByCodeMatch(
                        codeHex, 
                        isDeployed: tracepoint.ContractDeployed,
                        out info);
                    _getInstructionAndSourceMapCache[(xxhash, tracepoint.ContractDeployed)] = (ret, info);
                }
                ...
                var bytecodeInfo = _solcData.SolcBytecodeInfo.Single(s => (
                    s.FilePath == info.FilePath && 
                    s.ContractName == info.ContractName && 
                    s.BytecodeDeployed.Equals(info.BytecodeDeployed, StringComparison.Ordinal)));
        ...
        private void ParseStateVariableDeclarations()
        {
            ...
            var astContractNodes = AstParser.ContractNodes;
            
            foreach (var contractDefinitionNode in astContractNodes)
            {
        ...
        private void ParseScopeTracepoint(int traceIndex, SourceMapEntry currentEntry, ExecutionTraceScope currentScope)
        {
                        ...
                        for (int parameterIndex = 0; parameterIndex < inputParameterCount; parameterIndex++)
                        {
                            ...
                            if (!currentScope.CheckLocalVariableExists(parameter.Id))
                            {
                                ...
                                var localVariable = new LocalVariable(
                                    ...
                                    currentEntry)
                                {
                                    FunctionInputParameterCount = inputParameterCount,
                                    FunctionInputParameterIndex = parameterIndex
                                };
            ...
            if (currentScope.ScopeDepth > 0 && currentScope.ParentFunctionCall == null)
            {
                ...
                for (int previousTraceIndex = currentScope.StartIndex - 1; previousTraceIndex >= 0; previousTraceIndex--)
                {
                    ...
                    SourceMapEntry previousEntry;
                    
                    try
                    {
                        (previousInstructionNumber, previousEntry) = GetInstructionAndSourceMap(previousTraceIndex);
                    }
                    catch (Exception ex)
                    {
                        continue;
        ...
        private int ParseScopes(
            int traceIndex, 
            int scopeDepth, 
            int callDepth, 
            ExecutionTraceScope parentScope, 
            ref SourceMapEntry? lastEntry)
        {
            ...
            while (traceIndex < ExecutionTrace.Tracepoints.Length)
            {
                ...
                SourceMapEntry curSourceMapEntry;
                int instructionNumber;
                try
                {
                    (instructionNumber, curSourceMapEntry) = GetInstructionAndSourceMap(traceIndex);
                }
                catch (Exception e)
                {
                    var knownContracts = new HashSet<string>();
                    foreach (var k in _contractMapCache.Keys)
                    {
                        knownContracts.Add(k.ContractAddress);
                    }
                    
                    for (var i = traceIndex + 1; i < ExecutionTrace.Tracepoints.Length; ++i)
                    {
                        if (ExecutionTrace.Tracepoints[i].ContractAddress != null && 
                            knownContracts.Contains(ExecutionTrace.Tracepoints[i].ContractAddress.ToString()))
                        {
                            return i;
                        }
                    }
                ...
                var currentLines = GetSourceLines(curSourceMapEntry);
                bool significantStep = false;
                if (lastEntry.HasValue)
                {
                    // If our source maps are different.
                    if (!lastEntry.Value.Equals(curSourceMapEntry))
                    {
                ...
                bool enteringScope = currentInstructionIsCall || curSourceMapEntry.Jump == JumpInstruction.Function;
                ...
                bool exitingScope = !enteringScope && (callDepthDecreased || (!withinExternalCall && curSourceMapEntry.Jump == JumpInstruction.Return));
                ...
                else
                {
                    ...
                    ParseScopeTracepoint(traceIndex, curSourceMapEntry, currentScope);
                }
        ...
        public VariableValuePair[] GetLocalVariables(int traceIndex, IJsonRpcClient rpcClient = null)
        {
            ...
            foreach (var variable in currentScope.Locals.Values)
            {
                ...
                if (variable.VariableLocation == VarLocation.CallData ||
                    (variable.IsFunctionInputParameter && currentScope.FunctionDefinition.Visibility == AstTypes.Enums.AstDeclarationVisibility.External)) 
                {
                    try
                    {
                        value = variable.ValueParser.ParseFromCallData(ref callData, variable.FunctionInputParameterIndex);
                    }
                    catch (Exception ex)
                    {
        ...
        {
            // Obtain the scope for this trace index
            var currentScope = GetScope(traceIndex);
            ...
            // Define our result
            var result = new List<VariableValuePair>();
            ...
            var stateVariableDeclarations = AstParser.GetStateVariableDeclarations(currentScope.ContractDefinition);
