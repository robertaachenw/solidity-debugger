...
namespace Meadow.CoverageReport.Debugging.Variables.UnderlyingTypes
{
    public class VarArray : VarRefBase
    {
        ...
        public override object ParseFromCallData(ref Memory<byte> callData, int inputParameterIndex)
        {
            var strippedCallData = callData.Slice(4, callData.Length - 4);
            
            var startOffsetBytes = strippedCallData.Slice(
                inputParameterIndex * UInt256.SIZE, 
                UInt256.SIZE);
            
            var startOffset = (int)BigIntegerConverter.GetBigInteger(startOffsetBytes.Span, false, UInt256.SIZE);

            return ParseDereferencedFromMemory(strippedCallData, startOffset);
        }
